...
test.describe('Campaign List', () => {
  ...
  test('should display campaign list header', async ({ page }) => {
    ...
    await expect(
      page.getByText('Manage your advertising campaigns and track performance')
    ).toBeVisible();
  });
  ...
  test('should display search bar', async ({ page }) => {
    // Get the main search input (not the AG-Grid filter inputs)
  ...
  test('should display AG-Grid table', async ({ page }) => {
    // Wait for AG-Grid to load by checking for the root element
  ...
  test.skip('should handle API errors gracefully', async ({ page }) => {
    ...
    await expect(
      page.getByText(/Error loading campaigns: Request failed with status code 500/)
    ).toBeVisible();
  });
});
