...
docker exec bravo1_mongodb mongodump --db mediatool_v2 --out /data/backup-before-e2e-$(date +%Y%m%d-%H%M%S) > /dev/null 2>&1 || true

# Clear existing data
echo "🧹 Clearing existing data..."
docker exec bravo1_mongodb mongosh mediatool_v2 --eval "db.campaigns.deleteMany({})" > /dev/null
docker exec bravo1_mongodb mongosh mediatool_v2 --eval "db.lineItems.deleteMany({})" > /dev/null
docker exec bravo1_mongodb mongosh mediatool_v2 --eval "db.strategies.deleteMany({})" > /dev/null
