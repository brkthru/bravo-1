...
async function fixCampaignData() {
  ...
  try {
    ...
    samples.forEach((c) => {
      ...
      console.log(
        `  Dates: ${c.dates.start?.toLocaleDateString()} - ${c.dates.end?.toLocaleDateString()}`
      );
      console.log(
        `  Pacing: Delivery ${(c.metrics.deliveryPacing * 100).toFixed(1)}%, Spend ${(c.metrics.spendPacing * 100).toFixed(1)}%`
      );
    });
  } catch (error) {
