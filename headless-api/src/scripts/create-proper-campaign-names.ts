  ...
  try {
    ...
    for (const campaign of campaigns) {
      ...
      if (accountName !== 'Unknown Account') {
        ...
        const lineItemTypes = lineItemsForCampaign
          .map((li) => {
            const name = li.name || '';
            ...
            if (name.toLowerCase().includes('meta') || name.toLowerCase().includes('facebook'))
              return 'Social';
            if (name.toLowerCase().includes('ctv') || name.toLowerCase().includes('video'))
              return 'Video';
            return null;
          })
          .filter(Boolean);
