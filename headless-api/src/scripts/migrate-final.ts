...
async function migrateFinal() {
  ...
  try {
    ...
    console.log(
      `Loaded ${accounts.length} accounts, ${users.length} users, ${lineItems.length} line items`
    );
    ...
    for (const strategy of strategies) {
      ...
      if (strategyMediaPlans.length > 0) {
        const campaignIds = [
          ...new Set(strategyMediaPlans.map((mp) => mp.campaignId).filter(Boolean)),
        ];
    ...
    namedSamples.forEach((c) => {
      console.log(
        `- ${c.campaignNumber}: ${c.name} (${c.accountName}) - ${c.lineItemCount} line items, $${c.budget.total.toLocaleString()}`
      );
    });
  } catch (error) {
