...
import { generateId } from '@mediatool/shared';
...
class MigrationService {
  ...
  async migrateUsers() {
    ...
    const { rows: pgUsers } = await this.pgClient.query<PostgresUser>(`
  ...
  async migrateCampaigns(users: User[]) {
    ...
    const { rows: pgCampaigns } = await this.pgClient.query<PostgresCampaign>(`
    ...
    for (const pgCampaign of pgCampaigns) {
      ...
      const now = new Date();
      const daysElapsed = Math.floor((now.getTime() - pgCampaign.start_date.getTime()) / (1000 * 60 * 60 * 24));
      const totalDuration = Math.floor((pgCampaign.end_date.getTime() - pgCampaign.start_date.getTime()) / (1000 * 60 * 60 * 24));

      // Get user objects
      const leadAM = userMap.get(pgCampaign.lead_account_manager_id) || users[0];
      const mediaTrader = pgCampaign.media_trader_id ? userMap.get(pgCampaign.media_trader_id) : undefined;
      ...
      const campaign = {
        ...
        metrics: {
          ...
          spendPacing: (daysElapsed / totalDuration) > 0 ? (pgCampaign.budget_spent / pgCampaign.budget_total) / (daysElapsed / totalDuration) : 0,
...
`;
    ...
    await fs.writeFile('/Users/ryan/code-repos/brkthru/bravo_code/media-tool/bravo-1/bravo-1/API-DOCUMENTATION.md', docs);
