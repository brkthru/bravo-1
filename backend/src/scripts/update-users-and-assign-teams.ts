...
async function updateUsersAndAssignTeams() {
  ...
  try {
    ...
    const usersWithoutRoles = await db.collection('users').find({ 
      $or: [
        { role: { $exists: false } },
        { role: null },
        { role: 'undefined' }
    ...
    const accountManagers = await db.collection('users').find({
    ...
    console.log(`Found ${accountManagers.length} account managers and ${mediaTraders.length} media traders`);
    ...
    for (let i = 0; i < campaignUpdates.length; i += 100) {
      ...
      console.log(`Updated ${Math.min(i + 100, campaignUpdates.length)}/${campaignUpdates.length} campaigns...`);
    }
    ...
    samplesWithTeams.forEach(c => {
      console.log(`\n- ${c.name}`);
      console.log(`  Account Manager: ${c.team.leadAccountManager.name} (${c.team.leadAccountManager.email})`);
