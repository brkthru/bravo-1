...
async function analyzeCampaignData() {
  ...
  try {
    ...
    const campaignsWithLineItems = await db.collection('campaigns').find({
    ...
    const campaignsWithStrategy = await db.collection('campaigns').find({
    ...
    const campaignsWithDates = await db.collection('campaigns').find({
      $or: [
        { 'dates.start': { $exists: true } },
        { 'strategy.dates.start': { $exists: true } }
      ]
    }).limit(5).toArray();
