...
async function analyzeDataStructure() {
  ...
  try {
    ...
    const strategiesWithLineItems = await db.collection('strategies').aggregate([
      { $lookup: {
        from: 'lineItems',
        localField: '_id',
    ...
    if (strategiesWithLineItems.length > 0) {
      ...
      console.log('Line items:', strategy.lineItems.map(li => ({
        name: li.name,
        price: li.price,
    ...
    const mediaBuys = await db.collection('mediaBuys').find({
      $and: [
