...
async function migrateWithRealMappings() {
  ...
  try {
    ...
    for (const strategy of strategies) {
      ...
      if (team && team.ownerId) {
        ...
        if (owner) {
          leadAccountManager = {
            id: owner._id,
            name: owner.name || `${owner.firstName || ''} ${owner.lastName || ''}`.trim() || 'Account Manager',
            email: owner.email
          };
        }
      }
      ...
      if (mediaTraderIds.length > 0) {
        ...
        if (trader) {
          mediaTrader = {
            id: trader._id,
            name: trader.name || `${trader.firstName || ''} ${trader.lastName || ''}`.trim() || 'Media Trader',
            email: trader.email
          };
        }
      }
      ...
      if (lineItemsForStrategy.length > 0) {
        const starts = lineItemsForStrategy.filter(li => li.startDate).map(li => new Date(li.startDate));
        const ends = lineItemsForStrategy.filter(li => li.endDate).map(li => new Date(li.endDate));
      ...
      const deliveryPacing = expectedProgress > 0 ? (totalSpent / totalBudget) / expectedProgress : 1.0;
    ...
    // Show statistics
    const stats = await db.collection('campaigns').aggregate([
      {
        $group: {
          _id: null,
    ...
    samples.forEach(c => {
      ...
      console.log(`  Account: ${c.accountName}, Budget: $${c.budget.total.toLocaleString()}, Status: ${c.status}, Activity: ${c.mediaActivity}`);
      console.log(`  Team: ${c.team.leadAccountManager.name}${c.team.mediaTrader ? ' + ' + c.team.mediaTrader.name : ''}`);
    });
