...
import { Campaign, CampaignStatus } from '@mediatool/shared';
...
describe('CampaignModel', () => {
  ...
  describe('pacing calculations', () => {
    test('should show ~20% of campaigns as over-pacing', async () => {
      ...
      const overPacingCount = results.filter(c => 
        (c.metrics?.deliveryPacing || 0) > 1.0
      ).length;
    ...
    test('should ensure spend pacing is usually lower than delivery pacing', async () => {
      ...
      const lowerSpendCount = results.filter(c => 
        (c.metrics?.spendPacing || 0) <= (c.metrics?.deliveryPacing || 0)
      ).length;
