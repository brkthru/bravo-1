...
export const PacingScheduleSchema = z.object({
  _id: ObjectIdSchema,
...
export const PacingScheduleInputSchema = z.object({
  lineItemId: ObjectIdSchema,
  budgetBlocks: z.array(BudgetBlockSchema).min(1),
  notes: z.string().optional(),
}).refine(
  (data) => validateBudgetBlocks(data.budgetBlocks),
  'Budget blocks must not overlap'
);
