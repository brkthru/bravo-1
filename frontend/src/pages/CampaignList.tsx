...
import { Campaign, formatCurrency, formatPercentage } from '@bravo-1/shared';
import StatusBadge from '../components/ui/StatusBadge';
import ProgressBar from '../components/ui/ProgressBar';
import LoadingSpinner from '../components/ui/LoadingSpinner';
import CampaignDetail from '../components/CampaignDetail';
import { useTheme } from '../contexts/ThemeContext';
import { lightTheme, darkTheme } from '../themes/agGridTheme';
import { ConfirmationDialog } from '../components/ui/headless';
...
const TeamCellRenderer = ({ data }: { data: Campaign }) => {
  ...
  if (!data.team || !data.team.leadAccountManager) {
    return <div className="text-gray-500 text-sm">No team assigned</div>;
  }

  const { leadAccountManager, mediaTrader } = data.team;
  const initials = leadAccountManager.name
    .split(' ')
    .map((n) => n[0])
    .join('');
  ...
  const traderAvatar = mediaTrader
    ? `https://api.dicebear.com/9.x/miniavs/svg?seed=${encodeURIComponent(mediaTrader.name)}`
    : null;
...
const MediaActivityCellRenderer = ({ value }: { value: string }) => {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium transition-all ${colorMap[value] || 'bg-gray-100 text-gray-800'}`}
    >
...
export default function CampaignList() {
  ...
  const handleMediaStrategy = useCallback((campaign: Campaign) => {
    // Check if media strategy is initialized based on price allocation
    const hasMediaStrategy = campaign.price.actualAmount > 0;

    setConfirmDialog({
      isOpen: true,
      campaign,
      action: hasMediaStrategy ? 'edit' : 'init',
    });
  }, []);

  const handleConfirmMediaStrategy = useCallback(() => {
    const { campaign, action } = confirmDialog;
    if (!campaign) return;

    if (action === 'edit') {
      console.log('Edit media strategy for campaign:', campaign.campaignNumber);
      // TODO: Navigate to edit media strategy page
    } else {
      console.log('Initialize media strategy for campaign:', campaign.campaignNumber);
      // TODO: Navigate to initialize media strategy page
    }
  }, [confirmDialog]);
  ...
  const columnDefs = useMemo<ColDef[]>(
    () => [
        ...
        cellRenderer: ({ data }: { data: Campaign }) => (
          <div className="cursor-pointer hover:text-primary-600 dark:hover:text-primary-400 py-1">
            ...
            <div className="flex items-center gap-3 mt-0.5">
              ...
              <span className="text-xs text-gray-400 dark:text-gray-500">•</span>
              <span className="text-xs text-gray-500 dark:text-gray-400">
                {formatCurrency(data.price.targetAmount)} price
              </span>
              {data.dates && (
                <>
                  <span className="text-xs text-gray-400 dark:text-gray-500">•</span>
      ...
      },
      {
        ...
        cellRenderer: (params: any) => {
          const hasMediaStrategy = params.data.price.actualAmount > 0;
          return (
            <button
              onClick={() => handleMediaStrategy(params.data)}
            ...
            >
              ...
              {hasMediaStrategy ? 'Edit Strategy' : 'Init Strategy'}
            </button>
          );
        },
        sortable: false,
    ...
    [resolvedTheme, handleMediaStrategy]
  );
  ...
  const stats = useMemo(() => {
    ...
    const totalBudget = campaigns.reduce((sum, c) => sum + c.price.targetAmount, 0);
    const totalSpent = campaigns.reduce((sum, c) => sum + c.price.actualAmount, 0);
    const activeCampaigns = campaigns.filter(
      (c) => c.displayStatus !== 'Draft' && c.displayStatus !== 'Completed'
    ).length;
  ...
  return (
    <div className="px-4 sm:px-6 lg:px-8 py-8 bg-gray-50 dark:bg-gray-900 min-h-full">
      ...
      <div className="sm:flex sm:items-center sm:justify-between">
        ...
        <div className="mt-4 sm:mt-0 flex items-center space-x-3">
          ...
          {selectedCampaign && (
            <button
              type="button"
              onClick={() => handleMediaStrategy(selectedCampaign)}
            ...
            >
              ...
              {selectedCampaign.price.actualAmount > 0
                ? 'Edit Media Strategy'
                : 'Initialize Media Strategy'}
            </button>
          )}
        </div>
      </div>
      ...
      {/* Campaigns table */}
      <div className="mt-8 flex flex-col" style={{ height: 'calc(100vh - 320px)' }}>
        <div className="bg-white dark:bg-gray-800 shadow-sm rounded-lg overflow-hidden border border-gray-200 dark:border-gray-700 flex-1">
                  ...
                  >
                    ...
                    >
                      ...
                      >
                        <path
                          fillRule="evenodd"
                          d="M10.293 15.707a1 1 0 010-1.414L14.586 10l-4.293-4.293a1 1 0 111.414-1.414l5 5a1 1 0 010 1.414l-5 5a1 1 0 01-1.414 0zm-6 0a1 1 0 010-1.414L8.586 10 4.293 5.707a1 1 0 111.414-1.414l5 5a1 1 0 010 1.414l-5 5a1 1 0 01-1.414 0z"
                          clipRule="evenodd"
                        />
                      </svg>
                    </button>
      ...
      {/* Media Strategy Confirmation Dialog */}
      <ConfirmationDialog
        isOpen={confirmDialog.isOpen}
        onClose={() => setConfirmDialog({ isOpen: false, campaign: null, action: 'init' })}
        onConfirm={handleConfirmMediaStrategy}
        title={
          confirmDialog.action === 'init' ? 'Initialize Media Strategy' : 'Edit Media Strategy'
        }
        message={
          confirmDialog.action === 'init'
            ? `Are you ready to initialize the media strategy for "${confirmDialog.campaign?.name}"? This will set up the initial price allocation and targeting parameters.`
            : `Do you want to edit the media strategy for "${confirmDialog.campaign?.name}"? You can modify price allocations, targeting, and scheduling.`
        }
        confirmText={confirmDialog.action === 'init' ? 'Initialize' : 'Edit'}
        variant="info"
      />
    </div>
  );
