...
import { Campaign } from '@bravo-1/shared';
import {
  CalendarIcon,
  ChevronLeftIcon,
  ChevronRightIcon,
...
export default function MySchedule() {
  ...
  const priorityTasks = campaigns
    .map((campaign: Campaign) => ({
      ...
      daysLeft: Math.ceil(
        (new Date(campaign.dates.end).getTime() - new Date().getTime()) / (1000 * 60 * 60 * 24)
      ),
    }))
    .sort((a: any, b: any) => {
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="mt-2 text-gray-600 dark:text-gray-400">
          Your personalized campaign cockpit for{' '}
          ...
          <div className="bg-gradient-to-r from-primary-50 to-primary-100 dark:from-primary-900/20 dark:to-primary-800/20 rounded-lg p-4">
            ...
            <p className="text-sm text-primary-700 dark:text-primary-300">
              ...
              <span className="font-semibold">
                {priorityTasks.filter((t: any) => t.priority === 'critical').length} critical
              </span>{' '}
              and{' '}
              <span className="font-semibold">
                {priorityTasks.filter((t: any) => t.priority === 'high').length} high priority
              </span>{' '}
              campaigns that need attention today.
            </p>
          </div>
        </div>
      </div>
